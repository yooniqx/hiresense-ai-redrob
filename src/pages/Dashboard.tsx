...
export default function Dashboard() {
  ...
  const { data: rankedCandidates = [], isLoading: rankedLoading } = useQuery({
  ...
  const rankedCount = rankedCandidates.length;
  const averageScore = stats?.average_score || 0;
  const topScore = topCandidate?.score || 0;

  // Show loading state
  if (candidatesLoading || statsLoading || rankedLoading) {
    return (
      <AppLayout>
  ...
  const recentActivity = rankedCandidates.slice(0, 5).map((candidate, index) => ({
    who: candidate.name,
    action: "was ranked for",
    role: candidate.current_role || "Position",
  ...
  return (
    <AppLayout>
      ...
      <section className="relative overflow-hidden rounded-3xl border border-border bg-card p-8 md:p-12 mb-8">
        ...
        <div className="relative max-w-3xl">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full glass text-xs mb-5">
            ...
            <span className="text-muted-foreground">• {totalCandidates} candidates analyzed</span>
          </div>
          ...
          <div className="mt-7 flex flex-wrap gap-3">
            <Link to="/candidates"><PrimaryButton icon={<Users className="h-4 w-4" />}>View Candidates</PrimaryButton></Link>
            <Link to="/job-description"><GhostButton icon={<FileText className="h-4 w-4" />}>Job Description</GhostButton></Link>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <StatCard label="Total Candidates" value={totalCandidates} icon={<Users className="h-4 w-4" />} accent="ember" />
        <StatCard label="Ranked Candidates" value={rankedCount} icon={<CheckCircle2 className="h-4 w-4" />} accent="flame" />
        <StatCard label="Average Score" value={`${Math.round(averageScore)}%`} icon={<Gauge className="h-4 w-4" />} accent="amber" />
        <StatCard label="Top Score" value={`${Math.round(topScore)}%`} icon={<Crown className="h-4 w-4" />} accent="ember" />
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="rounded-2xl border border-border bg-card p-6">
          ...
          <p className="text-xs text-muted-foreground mb-5">Best candidate from dataset</p>
          {topCandidate ? (
            <>
              <div className="relative rounded-xl p-5 gradient-ember glow-ember text-white">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    ...
                    <div className="text-xs opacity-90">{topCandidate.current_role}</div>
                  </div>
                </div>
                ...
                <Link to={`/candidates/${topCandidate.candidate_id}`} className="mt-4 inline-flex items-center gap-1 text-xs font-medium underline-offset-2 hover:underline">
              ...
              <div className="mt-4 grid grid-cols-3 gap-2 text-center text-xs">
                ...
                <div className="rounded-lg bg-surface-2 p-2">
                  <div className="font-display font-bold text-base">{topCandidate.experience_years || 0}y</div>
